...
Scenario: Add new Cloud Hub Configuration for VMWare
 	...
 	When I start new configuration and set all fields as "test","localhost","RESTAPIACCESS","OAv8xVUnH8WeO2h0qzU2CIdH+1CJbrJxssv95GF4skE=","vermont2","groundwork\optimus-qa-test","gwopt@321","5"
...
Scenario: Verify VMware test connection with invalid input
	 Given I am on the Cloud Hub page
	 When I select "VMWare" for new connection
	 When I start new configuration and set all fields as "test123","localhost345","RESTAPIACCESS123","testing123","vermont211","groundwork\optimus-qa-test-456","gwopt@3214567","7"
	 And I select the Storage View, Network View and Resource Pool View
	 And I save the connection
	 And test connection is done
...
Scenario: Verify VMware connection with special characters and long strings
	 Given I am on the Cloud Hub page
	 When I select "VMWare" for new connection
	 When I start new configuration and set all fields as "!@#$%^&*()_+abvfcdeswaqzxsedcf","localhost345","@#%$%","!@#$%^&*()_+abvfcdeswaqzxsedcf","!@#$%^&*()_+abvfcdeswaqzxsedcf","!@#$%^&*()_+abvfcdeswaqzxsedcf","*%&*%*&%(*%","7"
	 And I select the Storage View, Network View and Resource Pool View
	 And I save the connection
	 And test connection is done
	 Then verify connection is not established  
...
Scenario: Verify VMware connection with special characters in check interval field
	 Given I am on the Cloud Hub page
	 When I select "VMWare" for new connection
	 When I start new configuration and set all fields as "test","localhost","RESTAPIACCESS","OAv8xVUnH8WeO2h0qzU2CIdH+1CJbrJxssv95GF4skE=","vermont2","groundwork\optimus-qa-test","gwopt@321","*&^%"
	 And I select the Storage View, Network View and Resource Pool View
	 And I save the connection
...
Scenario: Verify error messages on entering special characters in Warning and Critical thresholds fields 
	Given I am on the Cloud Hub page
	When I select "VMWare" for new connection
	When I start new configuration and set all fields as "test345","localhost","RESTAPIACCESS","OAv8xVUnH8WeO2h0qzU2CIdH+1CJbrJxssv95GF4skE=","vermont2","groundwork\optimus-qa-test","gwopt@321","7"
	And I select the Storage View, Network View and Resource Pool View
	And I save the connection
	And test connection is done
